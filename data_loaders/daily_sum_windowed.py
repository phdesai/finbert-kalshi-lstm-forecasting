"""
daily_sum_windowed.py (formerly data_loader_88_sum.py)

Combines the summed daily sentiment aggregation of daily_sum.py with the
20:00 UTC market-close windowing of daily_avg_windowed.py.
...
def load_sentiment():
    print("  → load_sentiment() [20→20 UTC window, summed]")
    tmp = pd.read_csv(
        "data/3D_articles_new_version_hm.csv",
        usecols=["date","mean_positive","mean_neutral","mean_negative"],
        parse_dates=["date"]
    )
    # assign each article to its 20:00→20:00 window
    ...
    # sum per window
    daily = tmp.groupby("window_date", as_index=True)[
        ["sent_pos","sent_neu","sent_neg"]
    ].sum()

    # normalize so pos+neu+neg=1
    S = daily.sum(axis=1).replace(0,1)
...
def load_mag():
    print("  → load_mag() [20→20 UTC window, summed]")
    tmp = pd.read_csv(
    ...
    daily = tmp.groupby("window_date", as_index=True)[
        ["mag_pos","mag_neu","mag_neg"]
    ].sum()
    S = daily.sum(axis=1).replace(0,1)
    daily[["mag_pos","mag_neu","mag_neg"]] = daily.div(S, axis=0)
    daily.index.name = "date"
    return daily.sort_index().loc[:"2025-04-30"]


def load_tariff():
    print("  → load_tariff() [20→20 UTC window, summed]")
    tmp = pd.read_csv(
    ...
    daily = tmp.groupby("window_date", as_index=True)[
        ["tariff_pos","tariff_neu","tariff_neg"]
    ].sum()
    S = daily.sum(axis=1).replace(0,1)
    daily[["tariff_pos","tariff_neu","tariff_neg"]] = daily.div(S, axis=0)
    daily.index.name = "date"
    return daily.sort_index().loc[:"2025-04-30"]
