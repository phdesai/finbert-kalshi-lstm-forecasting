"""
daily_avg_windowed.py (formerly data_loader_88.py)

Same daily-average sentiment aggregation as daily_avg.py, but sentiment
timestamps are assigned to a "sentiment day" using a 20:00 UTC market-close
cutoff (_assign_window_date): Mon-Thu after 20:00 rolls to the next day,
Fri after 20:00 and all weekend activity rolls to the following Monday.
...
def _assign_window_date(ts: pd.Timestamp) -> pd.Timestamp:
    """
    Map a UTC timestamp into the "sentiment day" whose close at 20:00 UTC
    we want to predict:
      - Mon–Thu before 20:00 → same calendar day
      - Mon–Thu at/after 20:00 → next calendar day
      - Fri before 20:00 → Fri
      - Fri at/after 20:00 → Monday (Fri+3 days)
    ...
    # Sat/Sun → next Monday
    days_to_monday = 7 - wd
    return base + pd.Timedelta(days=days_to_monday)
...
def load_sentiment():
    print("  → load_sentiment() [20:00 UTC→20:00 UTC, Fri→Mon weekend]")
    tmp = pd.read_csv(
    ...
    daily = (
        tmp
        .groupby("window_date")[["mean_positive","mean_neutral","mean_negative"]]
        .mean()
        .rename(columns={
...
def load_mag():
    print("  → load_mag() [20:00 UTC→20:00 UTC, Fri→Mon weekend]")
    tmp = pd.read_csv(
...
def load_tariff():
    print("  → load_tariff() [20:00 UTC→20:00 UTC, Fri→Mon weekend]")
    tmp = pd.read_csv(
...
def get_datasets(
    ...
    ds_pre_tr  = ds_xy(  X_pre_tr,  y_pre_tr,  cfg["batch_size"], shuffle=True,  seed=cfg["seed"])
