"""
daily_avg.py (formerly data_loader.py)

Baseline data loader: aggregates sentiment scores to a simple daily mean,
with sentiment days assigned by naive calendar-date normalization (no
market-close windowing). Loads SPY price plus daily-average sentiment for
articles, Mag 7, and Tariff subsets, and the Kalshi-derived series.
...
def ds_xy(X, y, batch_size, shuffle=True, seed=42):
    ...
    return ds.batch(min(batch_size, len(y)), drop_remainder=True).prefetch(tf.data.AUTOTUNE)
...
def load_sentiment():
    ...
    daily = tmp.groupby("date", as_index=True).mean().rename(columns={
        "mean_positive":"sent_pos",
        "mean_neutral":"sent_neu",
        "mean_negative":"sent_neg"
    })
    # normalize so pos+neu+neg=1
    s = daily[["sent_pos","sent_neu","sent_neg"]].sum(axis=1).replace(0,1)
    daily[["sent_pos","sent_neu","sent_neg"]] = (
        daily[["sent_pos","sent_neu","sent_neg"]].div(s, axis=0)
    )
...
def get_datasets(
    ...
    feat_cols = []
    if use_sentiment:
        print("→ joining daily sentiment")
        sent = load_sentiment()
        gm = sent[["sent_pos","sent_neu","sent_neg"]].mean()
        # fillna with mean of each column or 0 
    ...
    # ─── KALSHI AS INPUT ───────────────────────────────────────────────────────
    # always load the raw series for baseline
    print("→ get_datasets: loading raw Kalshi for baseline")
    kalshi_df = load_kalshi()

    if use_kalshi:
        # when use_kalshi=True, also join it into the features
        print("→ get_datasets: joining Kalshi as input feature")
        df = df.join(kalshi_df, how="left")
        gm = kalshi_df["kalshi_price"].mean()
        df["kalshi_price"] = df["kalshi_price"].fillna(0)
    ...
    # now z-score each group in feat_cols
    ...
    # FIXED: Create numpy arrays for tuning
    X_pre_tr_np, X_pre_val_np = X_price[:n_pre], X_price[n_pre:]
    y_pre_tr_np, y_pre_val_np = y_all[:n_pre],    y_all[n_pre:]
    ...
    ds_pre_tr  = ds_xy(  X_pre_tr_np,  y_pre_tr_np,  cfg["batch_size"], shuffle=True,  seed=cfg["seed"])
    ds_pre_val = ds_xy(  X_pre_val_np, y_pre_val_np, cfg["batch_size"], shuffle=False)
    ...
    y_true     = np.exp(scalers["price"].inverse_transform(y_true_z)).flatten()
    ...
    return (
        ...
        # FIXED: Return numpy arrays for hyperparameter tuning
        X_pre_tr_np, y_pre_tr_np, X_pre_val_np, y_pre_val_np
    )