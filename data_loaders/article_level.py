"""
article_level.py (formerly data_loader_article.py)

Variant that keeps sentiment at article/row level rather than aggregating
to a single daily value (no groupby), using naive calendar-date
normalization (no market-close windowing).
...
# ------------------------------------------------------------------------
# loading & prep functions (raw sentiment, mag, tariff)
# ------------------------------------------------------------------------
...
def load_sentiment():
    ...
    df = tmp.set_index("date").sort_index()
    return df
...
def load_mag():
    ...
    df = tmp.set_index("date").sort_index().loc[:"2025-04-30"]
    return df
